  ...
  return (
    <div className="mx-auto max-w-3xl pb-12">
      ...
      <UL>
        ...
        <li><strong>Hlavní karty</strong> (Poptávky · Zakázky · Konstrukce · Dílna · Lidé; Report pro vedení, Správa jen admin) tě pustí na <strong>Přehled</strong> modulu. Které karty vidíš, řídí administrátor v <strong>Přístupových právech</strong>.</li>
      ...
      <UL>
        ...
        <li><strong>Přístupová práva k modulům</strong> — tlačítko <Code>🔐 Přístupová práva</Code> ve Správě otevře matici <em>oddělení × modul</em> (Poptávky / Zakázky / Konstrukce / Dílna / Lidé): zaškrtneš, které moduly dané oddělení vidí. U konkrétního zaměstnance lze v jeho profilu zvolit <strong>Vlastní nastavení</strong>, které přepíše oddělení. <strong>Administrátor vidí vždy vše</strong>; kartu Správa má vždy jen admin. Dokud není nic nastaveno, vidí všichni vše (zpětná kompatibilita).</li>
        ...
        <li><strong>👥 Karta Lidé:</strong> hlavní záložka se seznamem zaměstnanců (karty s barvou, oddělením a pozicí, hledání podle jména i pozice, neaktivní sbalení dole). Kdo záložku vidí, řídíš v <strong>Přístupových právech</strong> (modul „Lidé"). Detail zaměstnance zůstává otevíratelný dvojklikem z tabulí i bez této záložky.</li>
        <li><strong>🌴 Dovolená a absence na kartě zaměstnance:</strong> na kartě zaměstnance (dvojklik na osobu na tabulích, nebo Správa) jde přidat <strong>dovolenou, nemoc, lékaře či „můj den"</strong> (typ + od–do; zapisuje editor/admin). <strong>Po dobu dovolené nejde zaměstnance přiřadit k akci</strong> — Zakázky přiřazení, změnu termínu i nasazení jako náhradníka zablokují (nejde vynutit). Absence se zároveň ukazují v Ganttu Konstrukce.</li>
      ...
      <UL>
        ...
        <li><strong>Montáž / Demontáž:</strong> v detailu akce je sekce, kam přidáš montáž nebo demontáž (výběr typu, nepovinné označení „Zakázka", popis, termín od–do). Vytvoří se jako <strong>zakázka k akci</strong> – proto se objeví i na <strong>Tabuli a v Ganttu</strong> (s odznakem Montáž/Demontáž) a <strong>přiřadíš jí lidi stejně jako ostatním zakázkám k akci</strong> (klikni na ni → přiřaď pracovníky). Prázdné termíny převezmou rozsah akce. V detailu montáže/demontáže si <strong>milníky pojmenováváš sám</strong> (žádná předvolba) a nemá <strong>konstrukční projekty</strong> ani vnořenou sekci Montáž/Demontáž – v modulu <strong>Konstrukce</strong> se proto nezobrazuje.</li>
        ...
        <li><strong>Rychlé akce na Tabuli:</strong> na kartě akce jsou vedle data ikonky <Code>◆</Code> (přidat <strong>milník</strong>), <Code>＋</Code> (přidat <strong>zakázku k akci</strong>) a <Code>💬</Code> (přidat <strong>poznámku</strong>) – malé okénko vyřídí zápis bez otvírání detailu. U zakázek k akci je ◆ a 💬 (u montáže/demontáže s vlastním názvem milníku).</li>
        <li><strong>⏸ Pozastavení a ▶ obnovení akce:</strong> tlačítko <Code>❚❚</Code> u akce (Tabule, seznam Akce i detail; jen s právem zápisu) akci <strong>pozastaví</strong> – zadáš datum a povinný důvod. Pozastaví se <strong>i všechny zakázky k akci</strong>. Pozastavená akce je všude <strong>zašedlá/vybledlá</strong> (Tabule, seznam, Gantt). <strong>Lidé se uvolní</strong> pro jiné akce (kolize je nehlásí), ale u akce zůstávají napsaní. Tlačítko <Code>▶</Code> akci <strong>obnoví</strong>: konec se posune o zbývající dny a lidé se přiřadí zpět — pokud jsou mezitím <strong>obsazení jinde</strong>, okno ukáže konflikty a u každého vybereš <strong>náhradníka</strong> (převezme stejné období), nebo necháš člověka na obou akcích. <strong>Pozastavit jde i jednotlivá zakázka k akci</strong> — pak se pozastaví jen ona (hlavní akce a ostatní zakázky běží dál). Když jde celá akce do <strong>fakturace</strong> nebo se <strong>uzavře</strong> a má pozastavenou zakázku k akci, tato zakázka se <strong>vyjme a povýší na samostatnou akci</strong> (zůstane pozastavená, dokončí se později; její konstrukční podúkoly dostanou vlastní projekt). Pozastavení se <strong>propisuje i do Konstrukce</strong> – projekty a úkoly pozastavené akce/zakázky jsou tam zašedlé se štítkem „pozastaveno". Vše se zapisuje do historie.</li>
        ...
        <li><strong>Uvolnění dělníků do fakturace:</strong> jakmile akci pošleš <em>do fakturace</em> (je hotová – občas dřív), pracovníkům se <strong>ukončí přiřazení k dnešnímu dni</strong> (u hlavní akce i na jejích zakázkách k akci). <strong>Zůstávají v evidenci akce</strong> – v seznamu „Přiřazení pracovníci" u nich uvidíš, že na akci pracovali (dají se zpětně dohledat) – ale od zítřka mají <strong>volný termín</strong>, takže je jde hned nasadit jinam. (Kdo měl termín teprve v budoucnu a nezačal, se z akce odebere.)</li>
